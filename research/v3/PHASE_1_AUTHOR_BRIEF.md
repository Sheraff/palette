...
| **Robustness harness** (`src/robustness/`) | How much a palette moves under changes that should not matter: JPEG re-encode at three qualities (q92/q85/q75), 1-bit dither, and the matched real-rendition pairs already on disk. This is success criterion 1 measured directly. *(An earlier revision of this row also listed resize, 1-px crop and id relabeling — those perturbations were never implemented; corrected 2026-08-04, reviewer-ruled the relabeling result was a bug, not a finding.)* | see `src/robustness/` | in flight |
