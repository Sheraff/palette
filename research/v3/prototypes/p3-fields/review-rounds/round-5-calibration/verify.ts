/**
 * Round 5 (calibration) — the staging checklist, run as code. `VERIFY.md` records the run.
 *
 * Round 2's seven checks, with three this round adds:
 *
 ...
 *     basename of `imagePath` with any extension removed, and nothing else. Round 2's ids were
 *     round-scoped (`r2-item-N`) and round 4 retired that shape so the installer never rewrites ids.
 *     A round that emitted an ordinal id would pass every other check here, so this one is its own.
 *  9. **Determinism of the staging run.** The palettes ship from `run-coverage-220-0.4.1.jsonl`. A
 *     second, independent execution over the same set — `--no-cache`, `P3_DIAG` **on** — is in
 *     `run-coverage-220-0.4.1-diag.jsonl`, and every one of the 220 rows must be palette-identical.
 *     That is the "diagnostics do not change the answer" claim of `diagnostics.ts` and the "two runs
 *     of the same candidate over the same set produce byte-identical rows" claim of `run.ts`, both
 *     checked rather than cited, over the whole set and not only the nine.
 * 10. **The side-car is blinded.** No serialized value may contain the prototype's name, an arm
 *     label, a version string, or any mechanism vocabulary. The sidecar is the one file that reaches
 *     the reviewer, and a leaked `p3-fields` in a colour *name* would be as fatal as one in a field.
 *
...
/** Version strings, which carry no word boundary a regex can lean on. Plain substring. */
const BLINDING_FORBIDDEN_LITERAL = ["0.4.1", "0.4.0", "0.3.0", "9504846"]
...
/** The shipping run, indexed by the repo-relative path the items carry. */
...
const runRows = readRun("run-coverage-220-0.4.1.jsonl")
const diagRows = readRun("run-coverage-220-0.4.1-diag.jsonl")
...
// 9 — determinism of the staging run, over the whole set rather than the nine.
{
	let compared = 0
	const diverged: string[] = []
	for (const [path, row] of runRows) {
		const other = diagRows.get(path)
		if (other === undefined) {
	...
	report.push(
		`determinism  ${diverged.length === 0 ? "PASS" : "FAIL"}  ${compared}/${runRows.size} rows compared against the ` +
			`--no-cache P3_DIAG re-run  divergences=${diverged.length}`,
	)
	for (const problem of diverged.slice(0, 10)) failures.push(`determinism: ${problem}`)
}
...
for (const item of items) {
	...
	if (row === undefined || !row.ok) {
		problems.push("no successful run row for this image in the source run")
	} else {
