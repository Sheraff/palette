/**
 * Round 7 (calibration) — the staging checklist, run as code. `VERIFY.md` records the run.
 *
 * Round 6's ten checks, unchanged in intent, plus one new check (11) because the branch tip moved past
 * the fingerprinted commit while this round was being staged:
 *
 ...
 *  9. **Determinism of the staging runs.** Six items ship from `run-coverage-220-0.4.4.jsonl` and one
 *     from `run-demo-20-0.4.4.jsonl`. Each has an independent second execution over the same set —
 ...
 *     `run.ts`, both checked rather than cited, over the whole sets and not only the seven.
 ...
 * 11. **`fingerprint.gitCommit` names the code that actually ran, and `dirty: false` is true.** NEW this
 *     round, and it is not ceremony: the branch tip advanced from `affe3965` to an orchestrator commit
 *     (`a51b1d29`, `STATE.md` only) while these runs were executing, so "HEAD equals the fingerprint" is
 *     *false* and a check asserting it would have to be either wrong or switched off. What the
 *     fingerprint actually claims is that the **sources these runs read are `affe3965`'s sources**, so
 *     that is what is checked, in two parts: `git status --porcelain` over
 *     `research/v3/prototypes/p3-fields/src` and `research/v3/src` is empty (nothing uncommitted was in
 *     the tree the workers imported), and `git diff --name-only affe3965 HEAD` over those same two trees
 *     is empty (nothing committed since changed them either). Together those say the working tree is
 *     byte-identical to `affe3965` where the algorithm lives — exactly what a re-run must check out.
 *
...
const BLINDING_FORBIDDEN = [
	...
	"conflict",
	"ramp",
	// 0.4.4's own vocabulary, added this round: the branch, its watch, and the words this round's
	// ROUND.md uses for the classes. A side-car that named any of them would tell the reviewer which
	// class an item is in, which is the whole thing the blinding protects.
	"neutral",
	"branch",
	"twin",
	"margin",
...
const BLINDING_FORBIDDEN_LITERAL = [
	"0.4.4",
	"0.4.3",
	"0.4.2",
	"0.4.1",
...
const DIAG_RUNS = {
	coverage: readRun("run-coverage-220-0.4.4-diag.jsonl"),
	demo: readRun("run-demo-20-0.4.4-diag.jsonl"),
} as const
...
// 11 — the fingerprinted commit is the code that ran; `dirty: false` is a fact about the tree.
{
	const SOURCE_TREES = ["research/v3/prototypes/p3-fields/src", "research/v3/src"]
	const git = (...args: string[]): string => execFileSync("git", args, { cwd: WORKTREE, encoding: "utf8" }).trim()
	const uncommitted = git("status", "--porcelain", "--", ...SOURCE_TREES)
	const sinceFingerprint = git("diff", "--name-only", FINGERPRINT.gitCommit, "HEAD", "--", ...SOURCE_TREES)
	const head = git("rev-parse", "--short=8", "HEAD")
	const ok = uncommitted === "" && sinceFingerprint === ""
	report.push(
		`source-identity ${ok ? "PASS" : "FAIL"}  HEAD=${head}  fingerprint=${FINGERPRINT.gitCommit}  ` +
			`uncommitted-in-src=${uncommitted === "" ? 0 : uncommitted.split("\n").length}  ` +
			`src-files-changed-since-fingerprint=${sinceFingerprint === "" ? 0 : sinceFingerprint.split("\n").length}`,
	)
	if (uncommitted !== "") failures.push(`source-identity: uncommitted source changes:\n${uncommitted}`)
	if (sinceFingerprint !== "") {
		failures.push(`source-identity: sources changed since ${FINGERPRINT.gitCommit}:\n${sinceFingerprint}`)
	}
}
