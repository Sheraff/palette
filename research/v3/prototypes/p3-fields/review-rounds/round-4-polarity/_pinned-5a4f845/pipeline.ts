...
export async function extractPalette(imagePath: string): Promise<P3Result> {
	...
	for (let endsStep = 0; endsStep <= MAX_RANK_STEPS; endsStep += 1) {
		...
		record("prevalence", {
			...
			// Which rule settled the polarity, and which polarity the run was asked for. Both are
			// dev-only (`P3_FORCE_BG_POLARITY`, unset on every shipped path) and both are here so that
			// round-4's diff-verification can read "same e1/e2, swapped assignment" off the chain rather
			// than infer it. See `backgroundPolarityOverrideInUse` in `constants.ts`.
			decidedBy: ends.polarityDecidedBy,
