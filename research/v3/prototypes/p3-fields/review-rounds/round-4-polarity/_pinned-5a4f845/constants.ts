...
/**
 * Which lightness extreme of the field a run calls the **background**. `null` — the variable unset —
 * is the shipped rule: prevalence decides, and only when prevalence ties does the darker-end
 * convention decide.
 */
export type BackgroundPolarity = "dark" | "light"

/**
 * **Dev-only override of the background/surface polarity**, read from `P3_FORCE_BG_POLARITY`.
 *
 * ## What this exists for, and what it must not become
 *
 * `review-rounds/round-4-polarity` asks the reviewer a two-sided question — *which of these two
 * treatments suits this artwork?* — where the two sides differ in **which lightness extreme of the
 * field is the background**. The published side is whatever the shipped rule chose. The other side
 * does not exist without a way to ask the pipeline for the opposite answer, exactly as round 3's
 * ROUND.md named "a dev-only force-gradient path" as owed work for the same structural reason.
 *
 * ## It is a *replacement of step 5*, not a post-hoc swap of two hexes
 *
 * When set, this override substitutes for the **whole** of `chooseFieldEnds`'s step 5 — both the
 * prevalence comparison and the tie-band's darker-end convention — with a single L comparison:
 *
 * - `dark`  — the darker end (lower OKLab L) is the background. This is *literally* the tie-band
 *   convention, so on a cover where the tie band fired the shipped run and `dark` agree by
 *   construction, which is what makes the round's diff-verification a real check rather than a
 *   restatement.
 * - `light` — the lighter end is the background. The convention, inverted.
 *
 * An exact L tie between the two ends resolves to the near end under **both** values, which is the
 * same 0.1.0 fallback the shipped tie branch uses, so the override never introduces an
 * order-of-comparison dependence the shipped path does not have.
 *
 * Everything downstream is re-derived by the normal pipeline — the gradient's orientation (its first
 * stop *is* the background), the foreground's ordering (built against the background), the accent, the
 * collapse test, and `validatePalette`. Nothing here swaps two fields of a finished palette.
 *
 * ## Dev-only, in the strong sense
 *
 * No product build sets the variable; nothing on the shipping path reads it; the pinned copy this
 * function lives in is not `src/`. An unrecognised value **throws** rather than falling back, because a
 * typo in a measurement's environment must not quietly produce the baseline again and be read as
 * "the override does nothing" — the same contract as `edgeRankInUse` and `depthFloorModeInUse`.
 */
export function backgroundPolarityOverrideInUse(): BackgroundPolarity | null {
	const override = process.env.P3_FORCE_BG_POLARITY
	if (override === undefined || override.length === 0) return null
	if (override !== "dark" && override !== "light") {
		throw new Error(`P3_FORCE_BG_POLARITY must be "dark" or "light"; got ${override}`)
	}
	return override
}
