...
import {
	...
	backgroundPolarityOverrideInUse,
...
export type FieldEnds = Readonly<{
	...
	/**
	 * Which rule actually settled the polarity. Dev-only diagnostics; nothing reads it back.
	 * `forced-*` occurs only under `P3_FORCE_BG_POLARITY`, which no shipped path sets.
	 */
	polarityDecidedBy: "prevalence" | "tie-band-convention" | "forced-dark" | "forced-light" | "no-extent"
...
export function chooseFieldEnds(image: DecodedImage, fieldSet: Int32Array, step: number): FieldEnds {
	...
	if (extent === 0) {
		...
		return {
			...
			// The no-extent path never reaches step 5 at all: both ends are m, so no polarity is chosen
			// and the override has nothing to invert. A cover on this path cannot be a round-4 item.
	...
	// **Dev-only, and not present in `src/`.** `P3_FORCE_BG_POLARITY` replaces the whole of step 5 with
	// a single L comparison so that round-4 can put the two polarities side by side. `dark` *is* the
	// tie-band convention, so on a tie-band cover it reproduces the shipped answer; `light` inverts it.
	// With the variable unset `polarityOverride` is `null` and the three lines below are exactly what
	// 5a4f845 shipped. See `backgroundPolarityOverrideInUse` in `constants.ts`.
	const polarityOverride = backgroundPolarityOverrideInUse()
	...
	} else if (relativeGap >= BACKGROUND_PREVALENCE_TIE_BAND) {
	...
	return {
		...
		polarityDecidedBy: polarityOverride !== null
			? (polarityOverride === "dark" ? "forced-dark" : "forced-light")
			: tieBandFired
			? "tie-band-convention"
			: "prevalence",
		farIsBackground,
