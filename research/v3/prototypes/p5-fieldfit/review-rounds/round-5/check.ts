/**
 * Round-5 validation battery. Round-4 §5's checks (themselves round-3 §5's), re-run against the
 * round-5 payload, plus:
 *   - the push-fingerprint check this round needs (`p5-fieldfit-0.9.2`, `gitCommit 57f02ee0`,
 *     `dirty false`);
 *   - the four leak-scan tokens the round-5 brief added (`coherence`, `gate`, `identity`,
 *     `crimson`) and the three v0.9 version strings;
 *   - the returning-item accent assertions (`#8d2639` / `#009cff` / `#7f7ca7`), so the loud stop is
 *     re-checked at validation time and not only at build time;
 *   - NOTE lines (not checks) for the served-name eyeball pass: which names repeat across items and
 *     which single name covers two DIFFERENT hexes. Those are recorded, never auto-corrected.
 *
...
// The brief's LOUD STOP, re-asserted here against the built payload.
const EXPECT: Record<string, string> = {
	ab67616d0000b27300012525e62c7f45baf46c90: "#8d2639",
	ab67616d0000b27300125577fb06a6a8942d6547: "#009cff",
	ab67616d0000b27300094a786a28459646be9b20: "#7f7ca7",
}
check(
	"the three returning covers publish the accents ROUND.md / v9d-delta.json predicted",
	Object.entries(EXPECT).every(([id, hex]) => items.find((i: any) => i.itemId === id)?.palette.accent === hex),
	Object.entries(EXPECT).map(([id, hex]) => `${id.slice(-10)}=${items.find((i: any) => i.itemId === id)?.palette.accent}(want ${hex})`).join(" "),
)
...
check(
	"fingerprint is uniform and names v0.9.2 at 57f02ee0, dirty false",
	items.every(
		(i: any) =>
			i.variantId === "p5-fieldfit-0.9.2" &&
			i.fingerprint.algorithmVersion === "p5-fieldfit-0.9.2" &&
			i.fingerprint.preprocessingVersion === "sharp-0.33.5/srgb/no-resample" &&
			i.fingerprint.gitCommit === "57f02ee0" &&
...
const TOKENS = [
	"p5", "fieldfit", "field-fit", "v0.", "0.3.0", "0.5.1", "0.6.0", "0.8.2", "0.9.0", "0.9.1", "0.9.2",
	"explf", "nofield",
	"round-1", "round-2", "round-3", "round-4", "round-5", "round 2", "round 3", "round 4", "round 5",
	...
	// round-5 brief additions
	"coherence", "gate", "identity", "crimson",
]
...
// colour DATA (item 1's surface is #d7e2e4). Scanned two ways instead, as rounds 3 and 4 did.
...
// ---- NOTES for the served-name pass (§names in STAGING.md). Reported, never enforced.
console.log("\n--- served-name notes (not checks) ---")
const byName = new Map<string, { items: Set<number>; hexes: Set<string> }>()
items.forEach((i: any, k: number) => {
	for (const r of render[i.itemId].roles) {
		if (!byName.has(r.name)) byName.set(r.name, { items: new Set(), hexes: new Set() })
		byName.get(r.name)!.items.add(k + 1)
		byName.get(r.name)!.hexes.add(r.hex)
	}
})
console.log(`NOTE  ${byName.size} distinct names over ${N * 4} served role names`)
for (const [name, v] of byName) {
	if (v.items.size > 1) console.log(`NOTE  cross-item name collision: "${name}" on items ${[...v.items].join(", ")} (hexes ${[...v.hexes].join(", ")})`)
}
for (const [name, v] of byName) {
	if (v.hexes.size > 1) console.log(`NOTE  ONE NAME, TWO DIFFERENT HEXES: "${name}" -> ${[...v.hexes].join(", ")}`)
}
