// Input-only census for round-5 items 4-8 selection.
//
// Extends `round-4/select-fresh-covers.mjs`. The STATISTICS are round-3's, unchanged, character for
// character (round-4 copied them verbatim and so does this file — the `srgbToOklab` helper and the
// whole `stats()` body below are byte-identical to round-4's). Two things changed:
//
//   1. the EXCLUSIONS gained `p5-round4-fresh.txt`, so every cover round 4 drew (including the two
//      it served fresh and the item-5 nominee that the ruling later admitted) is out of the pool;
//   2. the program now ALSO censuses the 20 distinct covers rounds 1-4 actually SERVED, into a
//      second file. Those rows are reference points for item 8's diversity rule; they are NOT pool
//      members and can never be selected.
//
// POOL RULE (stated before any statistic was read) — round-4's, UNCHANGED and deliberately so:
//   every file in shards 01/, 02/, 03/ of the repository root, in shard order 01, 02, 03 and within
//   each shard in ascending filename (byte) sort. All three shards are taken whole.
//   Holding the pool identical to round 4's is the point: items 4 and 5 re-run round-4's and
//   round-3's gates verbatim, so a like-for-like next-best draw is only meaningful if the pool is
//   the same one. Whole shards also keep the pool free of any cut-off parameter.
//
...
//   - `data/coverage-set/coverage-set-1.json`, all 220 artworks — censused in round 3, and the
//     brief asks for covers from BEYOND it;
//   - demo-20, `p5-round2-fresh`, `p5-round3-fresh` AND `p5-round4-fresh` — every cover any earlier
//     P5 round drew, which is a superset of every cover any earlier P5 round served;
...
//
// Usage: node select-fresh-covers.mjs <census-out.json> <served-census-out.json>
...
const drawn = [
	...readSet("demo-20.txt"),
	...readSet("p5-round2-fresh.txt"),
	...readSet("p5-round3-fresh.txt"),
	...readSet("p5-round4-fresh.txt"),
]
...
// The covers rounds 1-4 actually SERVED, read out of their own `items.json` payloads rather than
// re-listed by hand. Reference points for item 8's diversity rule; never pool members.
const SERVED = [
	...new Set(
		["round-1", "round-2", "round-3", "round-4"].flatMap((r) =>
			JSON.parse(fs.readFileSync(WT + "/research/v3/prototypes/p5-fieldfit/review-rounds/" + r + "/items.json", "utf8")).map(
				(i) => i.imagePath,
			),
		),
	),
...
// The served-cover reference census. Same `stats()`, same decode path; these rows never enter the
// pool and `rank.mjs` uses them only as fixed points to measure distance FROM.
const servedRows = []
for (const rel of SERVED) {
	const abs = path.join(WT, rel)
	const s = await stats(abs)
	servedRows.push({ path: rel, stem: stem(rel), ...s })
}
fs.writeFileSync(process.argv[3], JSON.stringify(servedRows, null, 1))

console.log(
	"census rows:", rows.length, "of", scanned, "scanned",
	"| skipped holdout:", skippedHoldout,
	"coverage-set-1:", skippedCoverage,
	"already-drawn(demo-20+r2+r3+r4):", skippedDrawn,
	"unreadable:", skippedUnreadable,
	"| served reference rows:", servedRows.length,
)
