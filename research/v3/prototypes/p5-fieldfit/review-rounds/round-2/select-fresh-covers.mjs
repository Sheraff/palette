// Input-only census over coverage-set-1, for round-2 items 7 and 8 selection.
...
const holdoutPaths = new Set(holdout.artworks.flatMap((a) => (a.files ?? []).map((f) => f.path)))
...
const demoKeys = new Set(demoSet.map(artKey))
...
for (const a of cov.artworks) {
	if (holdoutIds.has(a.artworkId) || holdoutPaths.has(a.path)) continue
	if (demoKeys.has(artKey(a.path))) continue
...
console.log("census rows:", rows.length, "of", cov.artworks.length)
