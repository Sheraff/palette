...
**Counts did NOT move on 2026-08-04, in the SAM-runtime-downgrade pass: still 88 items, 70 open, 18
closed.** No row was added, closed or amended. **The numbers were generated by rerunning the rule
stated in the paragraph above rather than by a new one** — `###` headings in A, B, C and L, skipping
the three whose row id is followed by a parenthesis, closed iff the heading carries `~~` — and the
per-section split it gives is unchanged at A1–A19 = 19 (9 open, 10 closed) · B1–B46 = 46 (40 open, 6
closed) · C1–C13 = 13 (12 open, 1 closed) · L-a–L-j = 10 (9 open, 1 closed). **Old versus new is
88 / 70 / 18 against 88 / 70 / 18**, which is the first pass in this file's history where rerunning
the count changed nothing — worth stating, because the rule's whole point is that an unchanged number
should be *derived* and not assumed.

**What the pass was for, and the row it looked for and did not find.** The reviewer ruled on
2026-08-04 that SAM at runtime is **not wanted and not banned** — recorded as
`d-2026-08-04-sam-at-runtime-downgraded-to-unwanted-not-banned`, with the verbatim quote and the
reviewer's own emphasis on *open*. The four conjunctive conditions in `PHASE_0_DECISIONS.md` §6.1 are
unchanged and still bind anything that wants SAM at runtime; the dev-time use is endorsed by name.
This file was then swept for **any open row predicated on SAM-at-runtime being an open question**, in
any wording. **There is none.** The two rows that mention a SAM runtime cost at all — **B40** (the
distinct-file count for a precompute pass) and **B41** (whether concurrent MLX processes help or
thrash) — were opened and read in full and are **left untouched deliberately**: each already carries a
*"What this is NOT about"* bullet scoping itself to the **dev-time** sweep and disclaiming any bearing
on admissibility, and B40's says outright that "a row that shrank the sweep would not move SAM one
step closer to admissible". A ruling that changes no condition cannot invalidate a row that was never
leaning on one, so amending them would have been decoration. **Nothing this ruling touches was ever
tracked here**, which is the same shape as the `sam-determinism-1` sweep recorded four paragraphs
above and is recorded for the same reason: a null sweep that goes unwritten gets run again.
