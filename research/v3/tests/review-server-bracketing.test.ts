...
describe("bracketing round in the review server", () => {
	...
	before(async () => {
		...
		assert.equal(seeded, "bracketing-round-1")
	...
	it("serves the two colours and nothing else", async () => {
		const payload = await call(harness.base, "GET", "/api/bracketing/bracketing-round-1")
	...
	it("records an answer as an oracle-label the warehouse accepts", async () => {
		...
		const response = await call(
			harness.base,
			"PUT",
			`/api/bracketing/bracketing-round-1/items/${first.token}/answer`,
			{ answer: true },
		)
		...
		assert.equal(record.batch?.id, "bracketing-round-1")
	...
	it("supersedes an answer when the reviewer steps back and answers again", async () => {
		...
		const after_ = await call(harness.base, "GET", "/api/bracketing/bracketing-round-1")
	...
	it("refuses an unknown token and a non-boolean answer", async () => {
		const bad = await call(harness.base, "PUT", "/api/bracketing/bracketing-round-1/items/deadbeef/answer", { answer: true })
		assert.equal(bad.status, 404)
		const payload = await call(harness.base, "GET", "/api/bracketing/bracketing-round-1")
		const wrong = await call(
			harness.base,
			"PUT",
			`/api/bracketing/bracketing-round-1/items/${payload.body.items[1].token}/answer`,
			{ answer: "yes" },
		)
	...
	it("appears in the queue and releases through the normal flow", async () => {
		...
		const entry = queue.body.batches.find((batch: any) => batch.batchId === "bracketing-round-1")
		...
		const refused = await call(harness.base, "POST", "/api/batches/bracketing-round-1/release")
		...
		for (const item of payload.body.items) {
			...
			await call(harness.base, "PUT", `/api/bracketing/bracketing-round-1/items/${item.token}/answer`, { answer: true })
		}
		const released = await call(harness.base, "POST", "/api/batches/bracketing-round-1/release")
		...
		assert.ok(isBatchReleased(harness.records(), "bracketing-round-1"))

		const closed = await call(
			harness.base,
			"PUT",
			`/api/bracketing/bracketing-round-1/items/${payload.body.items[0].token}/answer`,
			{ answer: false },
		)
	...
	it("rebuilds every answer after a restart", async () => {
		const before_ = await call(harness.base, "GET", "/api/bracketing/bracketing-round-1")
