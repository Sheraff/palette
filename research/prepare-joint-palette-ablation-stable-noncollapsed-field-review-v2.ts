...
const CANDIDATE_ALGORITHM_VERSION = "joint-palette-ablation-stable-noncollapsed-field-0.1.0-development"
...
const implementationFiles = [
	...
	"research/prepare-joint-palette-ablation-stable-noncollapsed-field-review-v2.ts",
...
if (process.argv.slice(2).length > 0) {
	throw new Error("prepare-joint-palette-ablation-stable-noncollapsed-field-review-v2.ts does not accept arguments")
}
...
const manifest = parse<{
	experimentId: string
	protocol: { authorization: { targetedCompleteChangedSetReviewAuthorizedIfStoppingRulesPass: boolean } }
}>("manifest.json")
const analysis = parse<{
	...
	changes: { oneAtom: number; twoAtoms: number; oneBlock: number }
...
if (manifest.experimentId !== EXPERIMENT_ID || analysis.experimentId !== EXPERIMENT_ID ||
	frontier.experimentId !== EXPERIMENT_ID ||
	!manifest.protocol.authorization.targetedCompleteChangedSetReviewAuthorizedIfStoppingRulesPass ||
	!analysis.structural.pass || analysis.structural.violationCount !== 0 || !analysis.review.authorized ||
	!analysis.review.diagnosticOnly || analysis.disposition !== "ablation-stable-noncollapsed-field-review-ready" ||
	analysis.coverage.ablationStableNoncollapsedFieldCandidates !== 9 || analysis.coverage.exactReviewTransfers !== 0 ||
	analysis.coverage.freshReviewRequired !== 9 || analysis.states["distinct-flat"] !== 6 || analysis.states.gradient !== 3 ||
	analysis.changes.oneAtom !== 7 || analysis.changes.twoAtoms !== 2 || analysis.changes.oneBlock !== 9 ||
	frontier.entries.length !== analysis.review.freshBlindedComparisonsRequired || frontier.entries.length !== 9 ||
	frontier.entries.some((entry) => !entry.selected.changed || !entry.selected.admitted ||
		entry.selected.changedSemanticBlocks !== 1 ||
		(entry.selected.changedSemanticAtoms !== 1 && entry.selected.changedSemanticAtoms !== 2) ||
		entry.selected.fieldState === "collapsed" || entry.selected.objectiveDeltas.some((delta) => delta < -1e-12) ||
		!entry.selected.objectiveDeltas.some((delta) => delta > 1e-12) ||
		entry.changedRoles.some((role) => role !== "background" && role !== "surface") ||
		(entry.changedRoles.length === 0 && !entry.gradientChanged) ||
		JSON.stringify(entry.candidate.foreground.rgb) !== JSON.stringify(entry.baseline.foreground.rgb) ||
		JSON.stringify(entry.candidate.accent.rgb) !== JSON.stringify(entry.baseline.accent.rgb))) {
	throw new Error("Ablation-stable noncollapsed field review inputs disagree")
}
...
const entries: NextPaletteReviewEntry[] = ordered.map((entry, order) => {
	...
	return {
		...
		frontierSignature: `ablation-stable|canonical-overlay-block|${entry.selected.fieldState}`,
...
await Promise.all([
	writeExclusive(join(outputRoot, "plan.json"), jsonBytes({
		...
		reviewVersion: NEXT_PALETTE_REVIEW_VERSION,
		presentationVersion: NEXT_PALETTE_REVIEW_PRESENTATION_VERSION,
		status: "targeted-ablation-stable-noncollapsed-field-review-authorized",
		experimentId: EXPERIMENT_ID,
		generatedAt,
		...
		interpretationPolicy: {
			...
			pairedPreferenceTestsOneAblationStableNoncollapsedFieldChange: true,
...
process.stderr.write(`Prepared ${entries.length} blinded ablation-stable noncollapsed field comparisons at ${outputRoot}\n`)
