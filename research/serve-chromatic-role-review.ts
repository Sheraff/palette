...
import {
	...
	type ChromaticRoleReviewFeedbackStore,
...
if (!manifestArgument || !feedbackArgument || unexpected.length > 0 || !/^\d+$/.test(portArgument)) {
	throw new Error("Usage: serve-chromatic-role-review.ts <manifest.json> <feedback.json> [port]")
}
const port = Number(portArgument)
if (port < 1 || port > 65535) throw new Error("Port must be between 1 and 65535")

const projectRoot = fileURLToPath(new URL("..", import.meta.url))
const reviewRoot = fileURLToPath(new URL("./chromatic-role-review", import.meta.url))
...
for (const [route, file, type] of [
	...
	const expected = manifest.provenance.presentation[`research/chromatic-role-review/${file}`]
...
let feedback: ChromaticRoleReviewFeedbackStore
if (await exists(feedbackPath)) {
	feedback = parseChromaticRoleReviewFeedbackStore(JSON.parse(await readFile(feedbackPath, "utf8")) as unknown, manifest)
} else {
	feedback = { schemaVersion: 1, reviewVersion: CHROMATIC_ROLE_REVIEW_VERSION, manifestId: manifest.manifestId, entries: [] }
...
const publicReview = {
	...
	entries: manifest.entries.map(({ source, assignment: _assignment, cohort: _cohort, ...entry }) => ({
		...entry,
...
const server = createServer(async (request, response) => {
	try {
		...
		if (request.method === "POST" && url.pathname === "/api/feedback") {
			...
			const entry = parseChromaticRoleReviewFeedbackEntry(await requestBody(request), manifest, false)
			const write = async () => {
...
server.listen(port, "127.0.0.1", () => {
	process.stderr.write(`Chromatic role review: http://127.0.0.1:${port}/\n`)
})
