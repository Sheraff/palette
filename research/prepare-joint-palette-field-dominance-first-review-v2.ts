...
const CANDIDATE_ALGORITHM_VERSION = "joint-palette-field-dominance-first-0.1.0-development"
...
const implementationFiles = [
	...
	"research/prepare-joint-palette-field-dominance-first-review-v2.ts",
...
if (process.argv.slice(2).length > 0) {
	throw new Error("prepare-joint-palette-field-dominance-first-review-v2.ts does not accept arguments")
}
...
if (manifest.experimentId !== EXPERIMENT_ID || analysis.experimentId !== EXPERIMENT_ID ||
	...
	!analysis.review.diagnosticOnly || analysis.disposition !== "targeted-field-dominance-review-ready" ||
	analysis.coverage.reviewCases !== 3 || analysis.coverage.strictDominatorAvailable !== 3 ||
	frontier.entries.length !== analysis.review.freshBlindedComparisonsRequired || frontier.entries.length !== 3 ||
	frontier.entries.some((entry) => entry.selected.route !== "strict-dominator" ||
		entry.selected.changedSemanticBlocks !== 1 || entry.selected.changedSemanticAtoms !== 2 ||
		...
		entry.changedRoles.length !== 1 || entry.changedRoles[0] !== "surface")) {
	throw new Error("Field dominance review inputs disagree")
}
...
const authorization = {
	...
	review: {
		authorized: true,
		diagnosticOnly: true,
		strictFieldDominatorsOnly: true,
...
const entries: NextPaletteReviewEntry[] = ordered.map((entry, order) => {
	...
	return {
		...
		frontierSignature: `field-dominance-first|strict-dominator|${entry.selected.fieldState}`,
...
await Promise.all([
	writeExclusive(join(outputRoot, "plan.json"), jsonBytes({
		...
		reviewVersion: NEXT_PALETTE_REVIEW_VERSION,
		presentationVersion: NEXT_PALETTE_REVIEW_PRESENTATION_VERSION,
		status: "targeted-strict-field-dominance-review-authorized",
		experimentId: EXPERIMENT_ID,
		generatedAt,
		...
		interpretationPolicy: {
			...
			pairedPreferenceTestsOneIsolatedFieldBlockChange: true,
...
process.stderr.write(`Prepared ${entries.length} blinded strict field dominance comparisons at ${outputRoot}\n`)
