/**
 * Byte-identity acceptance harness for the hygiene wave.
 *
 ...
 *     research/v2-3-experiments/adversarial-arch/sweep.ts --out <manifest.json> [--set base|scrambled|offpanel|quick|full]
 *
 *   node --no-warnings --experimental-strip-types \
 *     research/v2-3-experiments/adversarial-arch/sweep.ts --compare <before.json> <after.json>
 *
...
function offPanelEntries(): Entry[] {
	...
	const stride = Math.max(1, Math.floor(all.length / OFF_PANEL_SAMPLE))
	for (let index = 0; selected.size < OFF_PANEL_SAMPLE && index < all.length; index += stride) {
		const id = all[index]
...
function entriesFor(set: string): Entry[] {
	...
	if (set === "offpanel") return offPanelEntries()
	if (set === "quick") {
	...
	if (set === "full") return [...baseEntries(false), ...baseEntries(true), ...offPanelEntries()]
