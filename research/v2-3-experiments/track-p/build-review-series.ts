 ...
 *
 *   vivid-series-2-incumbent/<name>.json   the current winner, exactly as trunk produces it
 *   vivid-series-2-vivid/<name>.json       the SAME field/foreground with the accent replaced by
...
const COMMITTED = `${import.meta.dirname}/review-series-2`
...
for (const base of [OUT, COMMITTED]) {
	await mkdir(`${base}/vivid-series-2-incumbent`, { recursive: true })
	await mkdir(`${base}/vivid-series-2-vivid`, { recursive: true })
}
...
for (const caseFile of selection) {
	...
	// A flat treatment emits NO `researchRender` key at all. Series 1 emitted a
	// `{ kind: "solid" }` shape that is not in the schema and had to be stripped by hand.
	...
	const write = async (label: string, evaluation: typeof winner): Promise<void> => {
		...
		const body = `${JSON.stringify({
			...
			extraction: {
				...
				...(render(t.gradient) ? { researchRender: render(t.gradient) } : {}),
			},
		}, null, 1)}\n`
		for (const base of [OUT, COMMITTED]) await writeFile(`${base}/${label}/${name}.json`, body)
	}
	await write("vivid-series-2-incumbent", winner)
	await write("vivid-series-2-vivid", rival)
	...
	manifest.push({
		...
		name,
		cell: hit.cell,
		challengerWeakness: hit.weakness,
		hueGapDegrees: Number(hit.hueGapDegrees.toFixed(1)),
		sourceSha256: sha256,
		...
		vivid: {
			...
			observedComponentCount: vividFamily.observedComponentCount,
			totalComponentCount: vividFamily.componentCount,
			markComponentCount: vividFamily.markComponentCount,
...
const manifestBody = `${JSON.stringify({
	...
	trunk: "c2366d2",
	question: "Series 2: WHERE does the vividness preference stop? Batch 17 found it general (8 vivid / 3 equal / 0 incumbent), so this probes the edge.",
	cells: {
		boundary: "challenger progressively weaker — tinier, thinner chroma lead, thinner evidence. The last rungs are falsifier candidates.",
		"same-hue": "intensification rather than a new direction; the thin, mixed cell from batch 17",
		replicate: "ordinary hue jumps at Series-1 strength, as stability checks",
	},
	detectorSignature: {
		...
		populationRatio: 2,
		sameHueDegrees: 40,
		twoColourArtworksExcluded: "background family + foreground family >= accentBlend.minimumTwoColourCoverage (Track K2)",
	},
	labels: { incumbent: "vivid-series-2-incumbent", vivid: "vivid-series-2-vivid" },
	cases: manifest,
}, null, 1)}\n`
for (const base of [OUT, COMMITTED]) await writeFile(`${base}/vivid-series-2-manifest.json`, manifestBody)
