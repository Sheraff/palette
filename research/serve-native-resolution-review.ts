...
import {
	...
	NATIVE_RESOLUTION_REVIEW_PRESENTATION_VERSION,
...
type FeedbackEntry = {
	...
	timestamp: string
	reviewSchema: 2
	presentationVersion: typeof NATIVE_RESOLUTION_REVIEW_PRESENTATION_VERSION
	image: string
...
type FeedbackStore = {
	schemaVersion: 1
	experimentId: typeof NATIVE_RESOLUTION_EXPERIMENT_ID
	reviewIdentity: typeof NATIVE_RESOLUTION_REVIEW_ID
	presentationVersion: typeof NATIVE_RESOLUTION_REVIEW_PRESENTATION_VERSION
...
const artifactDirectory = join(researchRoot, "data", "experiments", NATIVE_RESOLUTION_EXPERIMENT_ID)
const feedbackPath = join(researchRoot, "data", "native-resolution-review-feedback.json")
...
const assignments = Object.fromEntries(artifact.reviewManifest.entries.map((entry) => [
	entry.file,
	entry.left === "baseline" ? ["previous", "spatial"] : ["spatial", "previous"],
]))
...
function validateFeedbackStore(value: unknown): asserts value is FeedbackStore {
	if (!isRecord(value) || value.schemaVersion !== 1 || value.experimentId !== NATIVE_RESOLUTION_EXPERIMENT_ID ||
		value.reviewIdentity !== NATIVE_RESOLUTION_REVIEW_ID ||
		value.presentationVersion !== NATIVE_RESOLUTION_REVIEW_PRESENTATION_VERSION ||
		...
		throw new Error("Native resolution feedback provenance is invalid")
	}
	...
	for (const entryValue of value.entries) {
		if (!isRecord(entryValue) || typeof entryValue.image !== "string" || seen.has(entryValue.image)) {
			throw new Error("Native resolution feedback entry is invalid")
		}
		...
		if (!queueEntry || !assignment || entryValue.reviewSchema !== 2 || entryValue.comparison !== "iteration" ||
			entryValue.presentationVersion !== NATIVE_RESOLUTION_REVIEW_PRESENTATION_VERSION ||
			...
			throw new Error(`Native resolution feedback entry failed validation: ${entryValue.image}`)
		}
	...
	try {
		...
		if (request.method === "POST" && url.pathname === "/api/feedback") {
			...
			if (!queueEntry || !assignment || input.comparison !== "iteration" || input.leftMethod !== assignment[0] ||
				...
				throw new Error("Feedback fields are invalid")
			}
...
server.listen(port, "127.0.0.1", () => {
	process.stderr.write(`Native resolution blinded review: http://127.0.0.1:${port}/\n`)
})
