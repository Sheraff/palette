...
const standaloneRoot = resolve(packageRoot, "research/v2-2")
...
test("runtime imports are closed inside v2-2 except declared npm dependencies", async () => {
	...
	for (const file of files) {
		...
		for (const specifier of specifiers) {
			...
			assert.ok(target.startsWith(`${standaloneRoot}${sep}`), `${file} escapes v2-2 via ${specifier}`)
		}
	}
})
