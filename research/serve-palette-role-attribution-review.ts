...
import {
	...
	parsePaletteRoleFeedbackStore,
	parsePaletteRoleManifest,
	type PaletteRoleFeedbackStore,
...
if (!manifestArgument || !feedbackArgument || unexpected.length > 0 || !/^\d+$/.test(portArgument)) {
	throw new Error("Usage: serve-palette-role-attribution-review.ts <manifest.json> <feedback.json> [port]")
}
const port = Number(portArgument)
if (port < 1 || port > 65535) throw new Error("Port must be between 1 and 65535")

const projectRoot = fileURLToPath(new URL("..", import.meta.url))
const reviewRoot = fileURLToPath(new URL("./palette-role-review", import.meta.url))
...
const manifest = parsePaletteRoleManifest(JSON.parse(await readFile(manifestPath, "utf8")) as unknown)
...
for (const [route, file, type] of [
	...
	const expected = manifest.provenance.presentation[`research/palette-role-review/${file}`]
	if (!expected || sha256(bytes) !== expected) throw new Error(`Presentation file changed after review preparation: ${file}`)
...
let feedback: PaletteRoleFeedbackStore
if (await exists(feedbackPath)) {
	feedback = parsePaletteRoleFeedbackStore(JSON.parse(await readFile(feedbackPath, "utf8")) as unknown, manifest)
} else {
	feedback = { schemaVersion: 2, reviewVersion: PALETTE_ROLE_REVIEW_VERSION, manifestId: manifest.manifestId, entries: [] }
...
const publicReview = {
	...
	entries: manifest.entries.map(({ source, developmentContext: _developmentContext, ...entry }) => ({
		...entry,
...
const server = createServer(async (request, response) => {
	try {
		...
		if (request.method === "POST" && url.pathname === "/api/feedback") {
			...
			const entry = parsePaletteRoleFeedbackEntry(await requestBody(request), manifest, false)
			const write = async () => {
				feedback.entries = [...feedback.entries.filter((stored) => stored.caseId !== entry.caseId), entry]
					.sort((first, second) => first.caseId.localeCompare(second.caseId, "en"))
...
server.listen(port, "127.0.0.1", () => {
	process.stderr.write(`Palette role attribution review: http://127.0.0.1:${port}/\n`)
})
