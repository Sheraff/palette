...
if (!manifestArgument || !feedbackArgument || !outputArgument || unexpected.length > 0) {
	throw new Error("Usage: analyze-next-palette-review.ts <manifest.json> <feedback.json> <analysis.json>")
}
...
process.stderr.write(`Analyzed complete next palette review batch ${manifest.batch.index}/${manifest.batch.totalBatches} at ${outputPath}\n`)
