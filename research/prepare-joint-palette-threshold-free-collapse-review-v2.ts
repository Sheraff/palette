...
const CANDIDATE_ALGORITHM_VERSION = "joint-palette-threshold-free-collapse-0.1.0-development"
...
const inputFiles = [
	...
	{ key: "review-transfer.json", path: join(experimentRoot, "review-transfer.json") },
...
const implementationFiles = [
	...
	"research/prepare-joint-palette-threshold-free-collapse-review-v2.ts",
...
type FrontierEntry = {
	...
	audit: { pass: boolean; clauses: Record<string, boolean> }
}
...
if (process.argv.slice(2).length > 0) {
	throw new Error("prepare-joint-palette-threshold-free-collapse-review-v2.ts does not accept arguments")
}
...
const manifest = parse<{
	experimentId: string
	protocol: { authorization: { targetedDiagnosticReviewAuthorizedIfStoppingRulesPass: boolean } }
}>("manifest.json")
...
const transfer = parse<{ experimentId: string; entries: unknown[] }>("review-transfer.json")
if (manifest.experimentId !== EXPERIMENT_ID || analysis.experimentId !== EXPERIMENT_ID ||
	frontier.experimentId !== EXPERIMENT_ID || transfer.experimentId !== EXPERIMENT_ID ||
	!manifest.protocol.authorization.targetedDiagnosticReviewAuthorizedIfStoppingRulesPass ||
	!analysis.structural.pass || analysis.structural.violationCount !== 0 || !analysis.review.authorized ||
	!analysis.review.diagnosticOnly || analysis.disposition !== "targeted-threshold-free-collapse-review-ready" ||
	analysis.coverage.auditedPassingCandidates !== 3 || analysis.coverage.exactPositiveTransfers !== 1 ||
	analysis.coverage.freshReviewRequired !== 2 || transfer.entries.length !== 1 ||
	frontier.entries.length !== analysis.review.freshBlindedComparisonsRequired || frontier.entries.length !== 2 ||
	frontier.entries.some((entry) => !entry.audit.pass || Object.values(entry.audit.clauses).some((value) => !value) ||
		entry.selected.fieldState !== "collapsed" || entry.selected.changedSemanticBlocks !== 1 ||
		entry.selected.changedSemanticAtoms !== 2 || entry.selected.objectiveDeltas.some((delta) => delta < -1e-12) ||
		!entry.selected.objectiveDeltas.some((delta) => delta > 1e-12) ||
		entry.changedRoles.length !== 1 || entry.changedRoles[0] !== "surface" || !entry.gradientChanged)) {
	throw new Error("Threshold-free collapse review inputs disagree")
}
...
const authorization = {
	...
	review: {
		authorized: true,
		diagnosticOnly: true,
		thresholdFreeAuditedCollapsesOnly: true,
...
const entries: NextPaletteReviewEntry[] = ordered.map((entry, order) => {
	...
	return {
		...
		frontierSignature: "threshold-free-collapse|four-relational-clauses|collapsed",
...
await Promise.all([
	writeExclusive(join(outputRoot, "plan.json"), jsonBytes({
		...
		reviewVersion: NEXT_PALETTE_REVIEW_VERSION,
		presentationVersion: NEXT_PALETTE_REVIEW_PRESENTATION_VERSION,
		status: "targeted-threshold-free-collapse-review-authorized",
		experimentId: EXPERIMENT_ID,
		generatedAt,
		scope: { freshBlindedComparisons: entries.length, exactPositiveTransfers: 1, totalBatches: 1, diagnosticOnly: true },
		interpretationPolicy: {
			completePaletteQualityIsDiagnostic: true,
			pairedPreferenceTestsOneExactSurfaceCollapse: true,
			allFourRelationalAuditClausesPassed: true,
...
process.stderr.write(`Prepared ${entries.length} blinded threshold-free collapse comparisons at ${outputRoot}\n`)
