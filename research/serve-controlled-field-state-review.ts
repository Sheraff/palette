...
import {
	...
	type ControlledFieldStateFeedbackStore,
} from "./src/controlled-field-state-review.ts"

const [manifestArgument, feedbackArgument, portArgument = "3130", ...unexpected] = process.argv.slice(2)
if (!manifestArgument || !feedbackArgument || unexpected.length > 0 || !/^\d+$/.test(portArgument)) {
	throw new Error("Usage: serve-controlled-field-state-review.ts <manifest.json> <feedback.json> [port]")
}
const port = Number(portArgument)
if (port < 1 || port > 65535) throw new Error("Port must be between 1 and 65535")

const projectRoot = fileURLToPath(new URL("..", import.meta.url))
const presentationRoot = fileURLToPath(new URL("./controlled-field-state-review", import.meta.url))
...
const manifest = parseControlledFieldStateManifest(JSON.parse(await readFile(manifestPath, "utf8")) as unknown)
const experimentRoot = resolve(dirname(manifestPath), "..")
for (const [file, expected] of Object.entries(manifest.provenance.experiment)) {
	if (sha256(await readFile(resolve(experimentRoot, file))) !== expected) {
...
for (const [route, file, type] of [
	...
	const bytes = await readFile(resolve(presentationRoot, file))
	const expected = manifest.provenance.presentation[`research/controlled-field-state-review/${file}`]
...
let feedback: ControlledFieldStateFeedbackStore
if (await exists(feedbackPath)) {
	feedback = parseControlledFieldStateFeedbackStore(JSON.parse(await readFile(feedbackPath, "utf8")) as unknown, manifest)
} else {
	feedback = { schemaVersion: 1, reviewVersion: CONTROLLED_FIELD_STATE_REVIEW_VERSION, manifestId: manifest.manifestId, entries: [] }
...
const publicReview = {
	...
	entries: manifest.entries.map(({ source, assignment: _assignment, ...entry }) => ({
		...entry,
...
const server = createServer(async (request, response) => {
	try {
		...
		if (request.method === "POST" && url.pathname === "/api/feedback") {
			...
			const entry = parseControlledFieldStateFeedbackEntry(await requestBody(request), manifest, false)
			const write = async () => {
...
server.listen(port, "127.0.0.1", () => {
	process.stderr.write(`Controlled field-state review batch ${manifest.batch.index}/${manifest.batch.totalBatches}: http://127.0.0.1:${port}/\n`)
})
