# Gradient Eligibility 0.8.5 Sealed Validation

## Purpose

The `05/` artwork set is independent validation data for frozen candidate `gradient-eligibility-0.8.5-dev`. Music artwork and `00/` through `04/` are development data. The `06/` and `07/` corpora are untouched reserves and are not inspected or bound by this round. No version 0.8.5 threshold or bound implementation file may change after the `05/` manifest is sealed.
...
- Families overlapping `00/` through `04/` by artwork identifier or exact byte hash are excluded.
