...
const [manifestArgument, feedbackArgument, portArgument = "3124", ...unexpected] = process.argv.slice(2)
if (!manifestArgument || !feedbackArgument || unexpected.length > 0 || !/^\d+$/.test(portArgument)) {
	throw new Error("Usage: serve-next-palette-review.ts <manifest.json> <feedback.json> [port]")
}
const port = Number(portArgument)
if (port < 1 || port > 65535) throw new Error("Port must be between 1 and 65535")

const projectRoot = fileURLToPath(new URL("..", import.meta.url))
const reviewRoot = fileURLToPath(new URL("./next-palette-review", import.meta.url))
...
for (const [route, file, type] of [
	...
	const expected = manifest.provenance.presentation[`research/next-palette-review/${file}`]
...
server.listen(port, "127.0.0.1", () => {
	process.stderr.write(`Next palette review batch ${manifest.batch.index}/${manifest.batch.totalBatches}: http://127.0.0.1:${port}/\n`)
})
