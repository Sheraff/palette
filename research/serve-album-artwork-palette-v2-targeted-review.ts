...
import {
	parseTargetedReviewSubmission,
	type TargetedReviewSubmission,
} from "./src/album-artwork-palette-v2-targeted-review.ts"
...
type StoredFeedback = TargetedReviewSubmission & Readonly<{ submittedAt: string }>
...
const reviewDirectory = resolve(moduleDirectory, "album-artwork-palette-v2-targeted-review-v3")
const archivedReviewDirectory = resolve(moduleDirectory, "album-artwork-palette-v2-targeted-review")
const manifestPath = resolve(moduleDirectory, "data/experiments/album-artwork-palette-v2-0.4.4-development/targeted-review-manifest.json")
...
async function readFeedback(manifest: ReviewManifest): Promise<FeedbackStore> {
	try {
		...
		if (parsed.schemaVersion !== 1 || parsed.reviewVersion !== manifest.reviewVersion || parsed.manifestId !== manifest.manifestId || !Array.isArray(parsed.entries)) {
			throw new Error("Feedback store does not match the current targeted review")
		}
		return parsed
	} catch (error: unknown) {
...
if (manifest.schemaVersion !== 1 || manifest.cases.length === 0 || manifest.cases.length > 4 ||
	manifest.cases.some(({ options }) => options.length < 2 || options.length > 4 || new Set(options.map(({ optionId }) => optionId)).size !== options.length)) {
	throw new Error("Run the 0.4.1 development analysis before starting targeted review")
}
...
const server = createServer(async (request, response) => {
	try {
		...
		if (request.method === "GET" && ["/v1-app.js", "/v1-styles.css"].includes(url.pathname)) {
			const filename = url.pathname === "/v1-app.js" ? "app.js" : "styles.css"
			const path = resolve(archivedReviewDirectory, filename)
			const bytes = await readFile(path)
			response.writeHead(200, headers(url.pathname.endsWith(".js") ? "text/javascript; charset=utf-8" : "text/css; charset=utf-8"))
			response.end(bytes)
			return
		}
		...
		if (request.method === "POST" && url.pathname === "/api/feedback") {
			...
			const submission = parseTargetedReviewSubmission(body, {
				caseId: reviewCase.caseId,
				sourceSha256: reviewCase.sourceSha256,
				optionIds: reviewCase.options.map(({ optionId }) => optionId),
			})
...
server.listen(port, "127.0.0.1", () => process.stdout.write(`Album artwork targeted review: http://127.0.0.1:${port}\n`))
