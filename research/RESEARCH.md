...
The 0.9 algorithm is frozen before holdout evaluation. Holdout entries are generated separately in `research/data/holdout-results.json`, excluded from feedback queues, and shown as a distinct gallery cohort. Aggregate hard gates, role-collapse rates, fallback rates, reconstruction diagnostics, and duplicate-resolution stability are reported in `research/data/holdout-summary.json`.

Individual holdout failures must not drive threshold changes without explicitly retiring those images from holdout status. Human visual evaluation should sample and freeze a subset before collecting judgments rather than reviewing only conspicuous failures found in the gallery.
