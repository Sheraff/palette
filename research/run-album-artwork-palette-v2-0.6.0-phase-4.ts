...
const EXPECTED_CANDIDATE_VERSION = "album-artwork-first-principles-0.6.0"
...
const EXPECTED_FREEZE_RAW_SHA256 = "d446d606fad926d30ecf702fc9c9c7605cd9444dc054d767aa1e7e91ab1080b8"
...
const futureRelativePath = ALBUM_ARTWORK_PALETTE_V2_FUTURE_SAMPLE_03_PATHS.sealedSample
const freezeRelativePath = "research/data/experiments/album-artwork-palette-v2-0.6.0-development/candidate-freeze.json"
const candidateChildRelativePath = "research/album-artwork-palette-v2-0.6.0-phase-4-candidate-child.ts"
const baselineChildRelativePath = "research/album-artwork-palette-v2-0.6.0-phase-4-baseline-child.ts"
const runnerRelativePath = "research/run-album-artwork-palette-v2-0.6.0-phase-4.ts"
const executionProtocolRelativePath = "research/ALBUM_ARTWORK_UI_PALETTE_PHASE_4_FUTURE_03_PROTOCOL.md"
...
async function verifiedFutureSample() {
	const [inventory, consumedSample02, consumptionReceipt02, protocolDocument, sealed] = await Promise.all([
		metadataInput(ALBUM_ARTWORK_PALETTE_V2_FUTURE_SAMPLE_03_PATHS.inventory),
		metadataInput(ALBUM_ARTWORK_PALETTE_V2_FUTURE_SAMPLE_03_PATHS.consumedSample02),
		metadataInput(ALBUM_ARTWORK_PALETTE_V2_FUTURE_SAMPLE_03_PATHS.consumptionReceipt02),
		requirePhysicalFile(ALBUM_ARTWORK_PALETTE_V2_FUTURE_SAMPLE_03_PATHS.protocolDocument),
		metadataInput(ALBUM_ARTWORK_PALETTE_V2_FUTURE_SAMPLE_03_PATHS.sealedSample),
	])
	if (sealed.rawSha256 !== EXPECTED_FUTURE_RAW_SHA256) throw new Error("Future sample raw hash changed")
	const input: FutureSample03BuildInput = {
...
function sourcesFromSample(sample: Awaited<ReturnType<typeof verifiedFutureSample>>["sample"]): SourceRecord[] {
	...
	if (sources.some(({ path }) => !/^11\/[A-Za-z0-9._-]+$/.test(path))) throw new Error("Future source is not a direct root-11 child")
	return sources
}
...
async function validateCandidateFreeze(raw: Buffer): Promise<Record<string, unknown>> {
	...
	if (!isRecord(freeze) || freeze.schemaVersion !== 1 || freeze.candidateVersion !== EXPECTED_CANDIDATE_VERSION ||
		...
		!isRecord(freeze.gates) || freeze.gates.mechanism !== true || freeze.gates.exactTwoCaseHumanDelta !== true ||
		...
		!isRecord(freeze.authorization) || freeze.authorization.futureSample03OneWayExecutionEligible !== true ||
		...
		throw new Error("Candidate freeze does not authorize future sample 03")
	}
...
async function main(): Promise<void> {
	...
	if (ALBUM_ARTWORK_PALETTE_V2_VERSION !== EXPECTED_CANDIDATE_VERSION) throw new Error("Candidate source version is not frozen 0.6.0")
	...
	const controls = await controlFileBindings([
		...
		"research/ALBUM_ARTWORK_UI_PALETTE_PROTOCOL_V2_0_6_0.md",
		ALBUM_ARTWORK_PALETTE_V2_FUTURE_SAMPLE_03_PATHS.protocolDocument,
		freezeRelativePath,
		"research/freeze-album-artwork-palette-v2-0.6.0.ts",
		futureRelativePath,
		ALBUM_ARTWORK_PALETTE_V2_FUTURE_SAMPLE_03_PATHS.inventory,
		ALBUM_ARTWORK_PALETTE_V2_FUTURE_SAMPLE_03_PATHS.consumedSample02,
		ALBUM_ARTWORK_PALETTE_V2_FUTURE_SAMPLE_03_PATHS.consumptionReceipt02,
		"research/src/album-artwork-palette-v2-phase-4-review.ts",
		"research/serve-album-artwork-palette-v2-phase-4-review.ts",
		"research/analyze-album-artwork-palette-v2-0.6.0-phase-4-review.ts",
	...
	const protocolIdentity = {
		...
		sourceCustody: {
			pathRule: "direct-root-11-child-only",
	...
	const reviewCases = assignments.map((assignment, index) => {
		...
		return {
			reviewCaseId: `future03-${String(index + 1).padStart(2, "0")}-${assignment.key.slice(0, 12)}`,
	...
	const provenanceIdentity = {
		schemaVersion: 1,
		reviewVersion: ALBUM_ARTWORK_PALETTE_V2_FUTURE_03_REVIEW_VERSION,
