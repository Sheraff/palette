...
function render() {
	...
	for (const [index, item] of payload.items.entries()) {
		...
		heading.append(
			create("p", { className: "eyebrow", text: "Comparison 01 / 01" }),
	...
	submitBlock.append(
		...
		create("button", { type: "submit", text: "Submit comparison" }),
	)
...
async function submit(event) {
	...
	if (responses.some((entry) => !QUALITY_VALUES.includes(entry.qualityA) ||
		...
		status.textContent = "Complete both absolute ratings and the relative judgment."
		return
	}
...
async function load() {
	try {
		...
		if (payload.schemaVersion !== 1 || payload.itemCount !== 1 || payload.items?.length !== 1 ||
