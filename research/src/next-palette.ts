...
export const NEXT_PALETTE_ALGORITHM_VERSION = "region-graph-next-0.1.0-dev"

export const NEXT_PALETTE_IDENTITY = Object.freeze({
	...
	objectiveFormulas: Object.freeze([
		...
		"distinct ? field support : 1 - max(field support of every distance-, foreground-APCA-, accent-APCA-, accent-distance-, and cardinality-feasible distinct surface)",
...
export type NextPalettePolicy = {
	...
	distinctFieldMinimumDistance: number
	distinctAccentFieldMinimumDistance: number
	maximumDistinctRoleColors: number
}

export const NEXT_PALETTE_DEVELOPMENT_POLICY: Readonly<NextPalettePolicy> = Object.freeze({
	version: "next-palette-policy-0.1.0-development",
...
export type NextPaletteObjectiveVector = readonly [
...
export type NextPaletteGradientState = "flat" | "gradient"
export type NextPaletteCandidateId = number | "generated-black" | "generated-white"

export type NextPaletteHardConstraintRejections = {
	...
	gradientLegality: number
	cardinality: number
}

export type NextPaletteCertificate = {
	schemaVersion: 1
	algorithmVersion: typeof NEXT_PALETTE_ALGORITHM_VERSION
	identity: typeof NEXT_PALETTE_IDENTITY
	...
	selected: {
		...
		objectives: NextPaletteObjectiveVector
...
export type NextPaletteResult = {
	palette: Palette
	certificate: NextPaletteCertificate
}

export type NextPaletteContext = NextPaletteResult & {
...
function assertPolicy(policy: NextPalettePolicy): Readonly<NextPalettePolicy> {
	...
	if (actualKeys.length !== expectedKeys.length || actualKeys.some((key, index) => key !== expectedKeys[index])) {
		throw new Error("Next palette policy must contain exactly the versioned consumer constraint fields")
	}
	if (typeof policy.version !== "string" || policy.version.length === 0) {
		throw new Error("Next palette policy version must be a non-empty string")
	}
	for (const [name, value] of Object.entries(policy)) {
		if (name === "version") continue
		if (typeof value !== "number" || !Number.isFinite(value) || value < 0) {
			throw new Error(`Next palette policy ${name} must be a finite non-negative number`)
		}
	}
	if (!Number.isInteger(policy.maximumDistinctRoleColors) || policy.maximumDistinctRoleColors < 1 ||
		...
		throw new Error("Next palette policy maximumDistinctRoleColors must be an integer within [1, 4]")
	}
...
export function solveNextPalette(
	graph: PaletteEvidenceGraph,
			...
			for (const foreground of foregrounds) {
				...
				for (const accent of accents) {
					for (const gradientState of gradientStates) {
						...
						const strongestDistinctField = collapsedSurface
							? fieldNodes.reduce((strongest, node) => {
								...
								return Math.max(strongest, fieldSupport(node, maximumPopulation))
							}, 0)
							: 0
						...
						const strongestDistinctAccent = accentCollapsed
							...
							: 0
						const objectives: NextPaletteObjectiveVector = [
							fieldSupport(backgroundNode, maximumPopulation),
							foregroundSupport(foreground, maximumPopulation),
							collapsedSurface ? 1 - strongestDistinctField : fieldSupport(surfaceNode, maximumPopulation),
							accentCollapsed
	...
	if (!best) throw new Error("No complete palette satisfies the next palette policy")
	const selectedApca = {
	...
	const certificate: NextPaletteCertificate = {
		schemaVersion: 1,
		algorithmVersion: NEXT_PALETTE_ALGORITHM_VERSION,
		identity: NEXT_PALETTE_IDENTITY,
		policy,
		counts,
...
export function extractNextPaletteWithContext(
	image: RawImage,
	...
	return deepFreeze({ ...solveNextPalette(graph, policy), perception, graph })
}

export function extractNextPalette(
	image: RawImage,
	policy: NextPalettePolicy = NEXT_PALETTE_DEVELOPMENT_POLICY,
): NextPaletteResult {
	const { palette, certificate } = extractNextPaletteWithContext(image, policy)
