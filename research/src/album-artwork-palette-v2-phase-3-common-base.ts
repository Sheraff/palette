...
import { buildBandLocalEndpointRefinements } from
	"./album-artwork-palette-v2-phase-3-endpoint-refinement.ts"
import type {
...
export type AlbumArtworkPaletteV2Phase3ModuleDiagnostics = Readonly<{
	version: string
}>

export type AlbumArtworkPaletteV2Phase3FieldProposalV2Input = Readonly<{
	evidence: NativePaletteEvidence
	gradientFits: readonly GradientFitDiagnostic[]
}>

export type AlbumArtworkPaletteV2Phase3FieldProposalV2Output = Readonly<{
	fieldHypotheses: readonly AlbumArtworkPaletteV2Phase3SourcedFieldHypothesis[]
	supplementalFamilies: readonly ColorFamilyEvidence[]
	diagnostics: AlbumArtworkPaletteV2Phase3ModuleDiagnostics
}>

export interface AlbumArtworkPaletteV2Phase3FieldProposalV2Module {
	propose(input: AlbumArtworkPaletteV2Phase3FieldProposalV2Input):
		AlbumArtworkPaletteV2Phase3FieldProposalV2Output
}

export type AlbumArtworkPaletteV2Phase3RoleDomainV2Input = Readonly<{
	evidence: NativePaletteEvidence
	fieldHypotheses: readonly AlbumArtworkPaletteV2Phase3SourcedFieldHypothesis[]
}>
...
export interface AlbumArtworkPaletteV2Phase3RoleDomainV2Module {
	construct(input: AlbumArtworkPaletteV2Phase3RoleDomainV2Input):
		AlbumArtworkPaletteV2Phase3RoleDomainV2Output
}

export type AlbumArtworkPaletteV2Phase3MaterializationV2Input = Readonly<{
	logicalDescriptors: readonly AlbumArtworkPaletteV2Phase3LogicalDescriptor[]
	identityObligations: readonly IdentityObligation[]
}>

export type AlbumArtworkPaletteV2Phase3MaterializationV2Output<TDiagnostics = AlbumArtworkPaletteV2Phase3ModuleDiagnostics> =
	Readonly<{
		materialized: readonly Readonly<{
			key: string
			treatment: CompletePaletteTreatment
...
export interface AlbumArtworkPaletteV2Phase3MaterializationV2Module<
	TOutput extends AlbumArtworkPaletteV2Phase3MaterializedDomainLike,
> {
	materialize(input: AlbumArtworkPaletteV2Phase3MaterializationV2Input): TOutput
}
...
> = Readonly<{
	evidence: NativePaletteEvidence
	identityObligations: readonly IdentityObligation[]
	materializedDomain: TDomain
}>

export interface AlbumArtworkPaletteV2Phase3SelectorV2Module<
	TDomain extends AlbumArtworkPaletteV2Phase3MaterializedDomainLike,
	TOutput,
> {
	select(input: AlbumArtworkPaletteV2Phase3SelectorV2Input<TDomain>): TOutput
}
