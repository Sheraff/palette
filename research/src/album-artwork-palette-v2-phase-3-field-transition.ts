...
import type { OKLab, RawImage } from "./types.ts"
...
export function buildNativeFieldTransitionDiscovery(image: RawImage): NativeFieldTransitionDiscovery {
	return discoverNativeFieldTransitions(buildNativePaletteEvidence(image))
}

export function buildNativeFieldTransitionHypotheses(evidence: NativePaletteEvidence): readonly FieldHypothesis[] {
	return discoverNativeFieldTransitions(evidence).hypotheses
}
