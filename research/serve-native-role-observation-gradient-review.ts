...
import {
	...
	NATIVE_ROLE_OBSERVATION_GRADIENT_REVIEW_PRESENTATION_VERSION,
...
type FeedbackEntry = {
	...
	timestamp: string
	reviewSchema: 2
	presentationVersion: typeof NATIVE_ROLE_OBSERVATION_GRADIENT_REVIEW_PRESENTATION_VERSION
	image: string
...
type FeedbackStore = {
	schemaVersion: 1
	experimentId: typeof NATIVE_ROLE_OBSERVATION_GRADIENT_ALGORITHM_VERSION
	reviewIdentity: typeof NATIVE_ROLE_OBSERVATION_GRADIENT_REVIEW_ID
	presentationVersion: typeof NATIVE_ROLE_OBSERVATION_GRADIENT_REVIEW_PRESENTATION_VERSION
...
const artifactDirectory = join(researchRoot, "data", "experiments", NATIVE_ROLE_OBSERVATION_GRADIENT_ALGORITHM_VERSION)
const feedbackPath = join(researchRoot, "data", "native-role-observation-gradient-review-feedback.json")
const artifact = await verifyNativeRoleObservationGradientArtifact(artifactDirectory, projectRoot)
const canonical = JSON.parse(await readFile(join(researchRoot, "data", "results.json"), "utf8"))
...
function validateFeedbackStore(value: unknown): asserts value is FeedbackStore {
	if (!isRecord(value) || value.schemaVersion !== 1 ||
		value.experimentId !== NATIVE_ROLE_OBSERVATION_GRADIENT_ALGORITHM_VERSION ||
		value.reviewIdentity !== NATIVE_ROLE_OBSERVATION_GRADIENT_REVIEW_ID ||
		value.presentationVersion !== NATIVE_ROLE_OBSERVATION_GRADIENT_REVIEW_PRESENTATION_VERSION ||
		...
		throw new Error("Native-role observation-gradient feedback provenance is invalid")
	}
	const seen = new Set<string>()
	for (const entry of value.entries) {
		if (!isRecord(entry) || typeof entry.image !== "string" || seen.has(entry.image)) {
			throw new Error("Native-role observation-gradient feedback entry is invalid")
		}
		const queueEntry = queueEntries.get(entry.image)
		const assignment = assignments[entry.image]
		if (!queueEntry || !assignment || entry.reviewSchema !== 2 || entry.comparison !== "iteration" ||
			entry.presentationVersion !== NATIVE_ROLE_OBSERVATION_GRADIENT_REVIEW_PRESENTATION_VERSION ||
			entry.leftMethod !== assignment[0] || entry.rightMethod !== assignment[1] ||
			entry.sourceSha256 !== queueEntry.sourceSha256 || entry.pairSha256 !== queueEntry.pairSha256 ||
			entry.manifestSha256 !== artifact.manifestSha256 ||
			typeof entry.preference !== "string" || !preferences.has(entry.preference as Preference) ||
			typeof entry.ship !== "string" || !shipDecisions.has(entry.ship as ShipDecision) ||
			!Array.isArray(entry.reasons) || entry.reasons.some((reason) => typeof reason !== "string" || !allowedReasons.has(reason)) ||
			typeof entry.note !== "string" || entry.note.length > 500 ||
			typeof entry.id !== "string" || typeof entry.timestamp !== "string") {
			throw new Error(`Native-role observation-gradient feedback entry failed validation: ${entry.image}`)
		}
		...
		if (request.method === "POST" && url.pathname === "/api/feedback") {
			...
			const entry = await serializeMutation(async (): Promise<FeedbackEntry> => {
				...
				const value: FeedbackEntry = {
					...
					previousAlgorithmVersion: canonical.algorithmVersion,
...
server.listen(port, "127.0.0.1", () => {
	process.stderr.write(`Native-role observation-gradient blinded review: http://127.0.0.1:${port}/\n`)
})
